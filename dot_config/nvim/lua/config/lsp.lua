...
local setup_formatting = function(client, buf)
	...
	local fmt = ok and conform or vim.lsp.buf

	local format = function()
		fmt.format({ bufnr = buf, id = client.id })
		-- HACK
	...
	map("", "<leader>lf", format, { desc = "Format buffer", buffer = buf })
	map("", "<leader>lt", function()
		local new_value = not vim.g.disable_autoformat
		vim.b[buf].disable_autoformat = new_value
		vim.g.disable_autoformat = new_value
		vim.notify("formatting " .. (vim.g.disable_autoformat and "disabled" or "enabled"))
	end, { desc = "Toggle formatting", buffer = buf })
	map("", "<leader>lT", function()
		vim.b[buf].disable_autoformat = not vim.b[buf].disable_autoformat
		vim.notify("buffer formatting " .. (vim.b[buf].disable_autoformat and "disabled" or "enabled"))
	end, { desc = "Toggle buffer formatting", buffer = buf })
end
...
local setup_diagnostics = function()
	...
	map("", "[e", function()
		prev_diag(severity.ERROR, "Previous error")
	end)
	map("", "]e", function()
		next_diag(severity.ERROR, "Next error")
	end)
	map("", "[w", function()
		prev_diag(severity.WARN, "Previous warning")
	end)
	map("", "]w", function()
		next_diag(severity.WARN, "Next warning")
	end)
